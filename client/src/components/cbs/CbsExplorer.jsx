...
export default function CbsExplorer({
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl flex-1 flex flex-col min-h-0 space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-800/90 border border-slate-700 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500"
            />
          </div>

          {loadingMahalle ? (
            <div className="py-12 text-center text-xs text-slate-400">Mahalleler yükleniyor...</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 overflow-y-auto pr-1 flex-1">
              {filteredMahalle.map((nh) => (
                <div
                  key={nh.objectid}
                  className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60 hover:border-amber-500/60 shadow-sm space-y-3 transition flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between">
                    ...
                    {/* Muhtar Bilgisi */}
                    <div className="mt-2 p-2.5 rounded-lg bg-slate-900/60 border border-slate-800 space-y-1 text-xs">
                      ...
                      {nh.muhtarTelefon && nh.muhtarTelefon !== '-' ? (
                        ...
                        <span className="text-slate-500 text-[11px]">Telefon kaydı yok</span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-1.5 mt-2.5 text-center text-[11px]">
                      <div className="bg-slate-900/40 p-1.5 rounded-lg">
                        <span className="text-slate-400 block text-[10px]">Bina</span>
                        <span className="font-bold text-white">{nh.yapiSayisi.toLocaleString('tr-TR')}</span>
                      </div>
                      <div className="bg-slate-900/40 p-1.5 rounded-lg">
                        <span className="text-slate-400 block text-[10px]">Kapı No</span>
                        <span className="font-bold text-white">{nh.kapiSayisi.toLocaleString('tr-TR')}</span>
                      </div>
                      <div className="bg-slate-900/40 p-1.5 rounded-lg">
                        <span className="text-slate-400 block text-[10px]">Yol</span>
      ...
      {activeSubTab === 'address' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl flex-1 flex flex-col min-h-0 space-y-4">
          
          {/* Adres Formu */}
          <form onSubmit={handleSearchAddress} className="space-y-3 bg-slate-800/50 p-4 rounded-xl border border-slate-700/60">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              
              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1">Mahalle</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1">Cadde / Sokak (CSBM)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1">Kapı No / Anahtar</label>
                <input
                  type="text"
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs sm:text-sm shadow-lg shadow-amber-500/20 transition flex items-center justify-center gap-2"
            >
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {loadingAddress ? (
              <div className="py-8 text-center text-xs text-slate-400">Sonuçlar getiriliyor...</div>
              ...
              addressResults.map((addr) => (
                <div
                  key={addr.objectid}
                  className="p-3 rounded-xl bg-slate-800/60 border border-slate-700/60 hover:border-slate-500 transition flex items-center justify-between text-xs"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-1.5 py-0.5 rounded bg-slate-900 text-amber-300 font-semibold text-[10px] border border-slate-700">
                        {addr.mahalle} Mah.
                      </span>
                    </div>
                    <div className="flex items-center gap-3 text-slate-400 text-[11px]">
                      <span>🏠 Mesken: {addr.meskenSayisi}</span>
                      <span>🏪 İşyeri: {addr.isyeriSayisi}</span>
                      <span>📅 Güncelleme: {addr.guncellemeTarihi}</span>
                    </div>
                  </div>
                  ...
                  {addr.latitude && addr.longitude && (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-md transition shrink-0 flex items-center gap-1"
                    >
      ...
      {activeSubTab === 'building' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl flex-1 flex flex-col min-h-0 space-y-4">
          ...
          {/* Bina Arama Formu */}
          <form onSubmit={handleSearchBuilding} className="space-y-3 bg-slate-800/50 p-4 rounded-xl border border-slate-700/60">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              
              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1">Mahalle Adı</label>
                <input
                  type="text"
                  placeholder="Örn: AKSARAY"
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1">Ada No</label>
                <input
                  type="text"
                  placeholder="Örn: 101"
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-300 block mb-1">Parsel No</label>
                <input
                  type="text"
                  placeholder="Örn: 5"
          ...
          {/* KVKK Uyarısı */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800/40 border border-slate-700/40 text-[11px] text-slate-400">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-1">
            {loadingBuilding ? (
              <div className="py-8 text-center text-xs text-slate-400">Bina detayları getiriliyor...</div>
              ...
              buildingResults.map((bina) => (
                <div
                  key={bina.objectid}
                  className="p-4 rounded-xl bg-slate-800/70 border border-slate-700/70 shadow-sm space-y-3"
                >
                  <div className="flex items-center justify-between border-b border-slate-700 pb-2">
                    ...
                    {bina.tabanAlaniM2 && (
                      <span className="px-2 py-1 rounded bg-slate-900 text-slate-300 font-mono text-xs border border-slate-700">
                        {bina.tabanAlaniM2} m²
                      </span>
                    )}
                  </div>
                  ...
                  {bina.photos && bina.photos.length > 0 && (
                    <div className="pt-2 border-t border-slate-700/60 space-y-1.5">
                      <span className="text-[11px] font-bold text-amber-400">
                        📷 Saha Fotoğrafları ({bina.photos.length})
                      </span>
                      <div className="grid grid-cols-3 gap-2">
                        {bina.photos.map((ph) => (
                          <a
                            ...
                            className="aspect-video rounded-lg overflow-hidden border border-slate-700 bg-black block group"
                          >
                            <img
                              src={ph.url}
                              alt={ph.name}
