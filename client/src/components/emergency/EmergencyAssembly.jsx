...
export default function EmergencyAssembly({ onSelectAreaOnMap, onAreasLoaded }) {
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      ...
      {/* 1. Üst Bilgi Kartı */}
      <div className="bg-gradient-to-r from-rose-950/80 via-zinc-900 to-zinc-900 border border-rose-900/40 rounded-2xl p-4 shadow-xl">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 shadow-xl space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-2">
          ...
          <div className="relative sm:col-span-7">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl bg-zinc-800/90 border border-zinc-700 text-xs sm:text-sm text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-rose-500"
            />
          </div>
          ...
          <div className="sm:col-span-5">
            <select
              ...
              className="w-full px-3 py-2 rounded-xl bg-zinc-800/90 border border-zinc-700 text-xs sm:text-sm text-white focus:outline-none focus:ring-2 focus:ring-rose-500"
            >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-zinc-800">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            >
              <span className="flex items-center gap-1"><PersonStanding className="w-3.5 h-3.5" /> WC</span>
            </button>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 shadow-xl flex-1 flex flex-col min-h-0 space-y-3">
        ...
        {loading ? (
          <div className="py-12 text-center text-xs text-zinc-400">Acil toplanma alanları yükleniyor...</div>
          ...
          <div className="p-8 text-center text-zinc-400 space-y-2">
            ...
            <p className="text-xs text-zinc-500">Lütfen filtreleri esnetin veya farklı bir mahalle seçin.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 overflow-y-auto pr-1 flex-1">
            {sortedAreas.map((area) => (
              <div
                ...
                className="p-4 rounded-xl bg-zinc-800/60 border border-zinc-700/60 hover:border-rose-500/60 shadow-sm space-y-3 transition flex flex-col justify-between"
              >
                <div>
                  ...
                  {/* Mahalle & Mevkii */}
                  <div className="flex items-center gap-1.5 text-xs text-zinc-300 mt-2">
                    <MapPin className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                    <span>{area.mahalle} Mah. {area.mevkii ? `— ${area.mevkii}` : ''}</span>
                  </div>

                  {/* Alan m2 & Donanımlar */}
                  <div className="grid grid-cols-2 gap-2 mt-3 pt-2 border-t border-zinc-700/60 text-xs">
                    <div>
                      <span className="text-zinc-400 block text-[10px]">Toplam Alan</span>
                    ...
                    <div>
                      <span className="text-zinc-400 block text-[10px]">Donanım Durumu</span>
                      <div className="flex items-center gap-2 mt-0.5 text-xs">
                        <span title="Engelli Uygun" className={area.engelliUygun ? 'text-emerald-400 font-bold' : 'text-zinc-600'}><Accessibility className="w-4 h-4" /></span>
                        <span title="Su" className={area.su ? 'text-sky-400 font-bold' : 'text-zinc-600'}><Droplets className="w-4 h-4" /></span>
                        <span title="WC" className={area.wc ? 'text-indigo-400 font-bold' : 'text-zinc-600'}><PersonStanding className="w-4 h-4" /></span>
                        <span title="Elektrik" className={area.elektrik ? 'text-amber-400 font-bold' : 'text-zinc-600'}><Zap className="w-4 h-4" /></span>
                      </div>
                    </div>
                  </div>
                ...
                {/* Aksiyon Butonları */}
                <div className="flex items-center gap-2 pt-2 border-t border-zinc-700/60">
                  <button
                  ...
                  {area.latitude && area.longitude && (
                    <a
                      ...
                      className="py-1.5 px-3 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold border border-zinc-700 transition flex items-center gap-1"
                    >
